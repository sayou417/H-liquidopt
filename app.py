    ...
    if st.session_state.get(
        ...
        if layout_source.empty:
            ...
            if not pod_network_summary.empty:
                ...
                # ===================================
                # Numerical Solver Quality Check
                # ===================================
                if (
                    not network_summary.empty
                    and "numerical_check_passed"
                    ...
                    check_col1, check_col2, check_col3 = (
                        st.columns(3)
                    )
                    ...
                    if numerical_passed:
                        st.success(
                            "✓ Nonlinear hydraulic solution passed "
                            "the PoC numerical residual check."
                        )
    
                    else:
                        st.warning(
                            "The nonlinear solution requires review "
                            "because one or more numerical residual "
                            "criteria were not satisfied."
                        )
    
                    st.caption(
                        "PoC numerical acceptance criteria: "
                        "maximum rack-path pressure residual ≤ 0.10% "
                        "and total-flow residual ≤ 0.01%. "
                        "These are prototype numerical screening "
                        "criteria, not industry certification limits."
                    )
